...
**Prefix:** LIF
**Status:** Layers 1–7 complete. Chapters C1–C5 complete; C6 stub.
...
Current delivery state — Layers 1–7 complete.
...
C4Container
  ...
  Container_Boundary(b5, "Layer: casehub-engine (L5)") {
    Container(casehubs, "TravelPlanCaseHub · HomeMaintenanceCaseHub · (6 more)", "LifeTypedCaseHub (app/engine/)", "8 YAML + DSL paired case definitions; FuncDSL workers; configureCase() augmentation hook")
...
| Auth retrofit readiness | `auth-retrofit-readiness.md` — auth wired (life#40): casehub-platform-oidc, @RolesAllowed on all REST resources |
...
`@RolesAllowed` is wired on all REST resources (life#40, casehub-platform-oidc). Permission topology: household-admin (full authority), household-member (standard), household-junior (view-only for tasks). `actorId` convention: `"life-system"` for system events; GDPR erasure actorId derived from `currentPrincipal.actorId()` (auth wired).
...
Layer 6 delivered: TrustRoutingPolicyProvider with 8 domain policies, 32-entry capability→domain mapping, attestation pipeline (LifeOutcomeAttestationWriter), ExternalActor REST response enriched with TrustProfile. Single-candidate routing until Layer 7 adds OpenClaw WorkerProvisioner.
...
- **Why `oversightKey` column:** resolved the `delegateTo` semantic overload (life#30). OVERSIGHT mode dedup keys now live in `oversightKey`; `delegateTo` is DELEGATION-only (principal ID).
...
**YAML + DSL paired case definition with LifeTypedCaseHub extension:** each case has a YAML definition (structure) and a DSL companion class extending `LifeTypedCaseHub` with `configureCase()` override. The `configureCase()` template method hook allows Java code to augment the YAML definition (add workers, set agent descriptors, bind M-of-N logic). Workers use `FuncWorkflowBuilder` per PP-20260531.

#### Pattern anchor

- `TravelPlanCaseHub#configureCase()` — YAML + DSL augmentation via `LifeTypedCaseHub` template method override; M-of-N SubCase bindings added in override
...
6. For each case: YAML at `app/src/main/resources/{app}/`, `@ApplicationScoped` subclass extending `LifeTypedCaseHub` with `configureCase()` override, and companion DSL class with static `build()`. The `configureCase()` hook augments the YAML definition (add workers, set agent descriptors, bind M-of-N logic).
...
**Architectural patterns:** Bayesian Beta trust scoring delegated to casehub-ledger (`ActorTrustScore`, `TrustGateService`); life provides routing policy via `TrustRoutingPolicyProvider` SPI + YAML overlay; attestation pipeline writes `LedgerAttestation` records from WorkItem outcomes.
**Key protocols:** PP-20260524-a8f597 (platform scope), `trust-routing.yaml` (YAML blend factors + quality floors per domain)
**Design refs:** `docs/specs/life-automation.md` §trust-dimensions, `docs/specs/life-actor-model.md`
**Issues:** casehubio/life#7
**Navigation:** `git log --grep="#7" --oneline`
**Blog:** `blog/2026-06-16-mdp01-snapshot-silent-alternatives.md`
**Improvement refs:** Single-candidate limitation until Layer 7 (OpenClaw WorkerProvisioner); RBAC-differentiated thresholds complete (life#26 — admin elevated, junior always-gate)
**Completed:** 2026-06-16

#### What it adds

Trust-weighted agent routing by household domain. `ActorTrustScore` updated from WorkItem outcomes via Bayesian Beta scoring in casehub-ledger. Routing selects the agent with the highest domain-specific trust score — `deadline-reliability` for contractor tasks, `cost-accuracy` for finance, `factual-accuracy` for health.

Life's contribution:

- `LifeOutcomeAttestationWriter` — converts WorkItem outcomes (COMPLETED → SOUND, SLA_BREACH → FLAGGED) and `deadline-reliability` dimension scores into `LedgerAttestation` records. Capability tag resolved from `LifeTaskContext.domain` or WorkItem scope path.
- `LifeTrustRoutingPolicyProvider` — `TrustRoutingPolicyProvider` SPI with 8 domain routing policies. Capability index built dynamically at `@PostConstruct` from `LifeDomainDescriptor.workerCapabilities()` — no static maps. YAML overlay (blend factors, quality floors) applied per domain scope.
- `trust-routing.yaml` — 8 domain scopes; blend factors range from 0.40 (household, family scheduling) to 0.80 (legal deadline); quality floors enforce minimum scores on specific trust dimensions.
- `TrustProfile` on `ExternalActorResponse` — ledger-backed global score, dimension scores, and capability scores exposed via REST. Score computed by `TrustGateService` (casehub-ledger) from `MaterializedTrustScoreSource` → `JpaActorTrustScoreRepository`.
- `LifeActorIds` — `life-actor:{uuid}` convention mapping ExternalActor UUIDs to ledger actor IDs.

**Design decision:** Bayesian Beta computation and materialized score tables are fully owned by casehub-ledger. Life only writes attestations and reads scores — it has no scoring logic of its own. This boundary prevents duplication and keeps trust computation consistent across all CaseHub applications.

**Single-candidate limitation (resolved in Layer 7):** AgentExec workers are single-candidate per capability. Layer 7 adds the WorkerProvisioner path with sentinel capabilities — the provisioner can resolve agents from multiple candidates using trust-weighted routing when the provisioner path grows beyond single-agent-per-domain.
...
**Blog:** `2026-06-18-mdp01-first-real-worker.md` (partial — AgentExec wiring)
**Improvement refs:** engine#536 (BlackboardEventCodecRegistrar idempotency — ✅ fixed 2026-06-19), engine#537 (CaseContextChangedEventHandler @Blocking — ✅ fixed 2026-06-18)
**Completed:** 🔲 pending (AgentExec partial ✅ 2026-06-18)

🔲 Full entry at C6 close. Life#25 delivered WorkerFunction.AgentExec(Agent) pattern with OpenClaw LLM backend. Full WorkerProvisioner wiring (life#37, life#38) still pending.
...
| **LifeTypedCaseHub** | casehub-life's typed extension of YamlCaseHub that provides the `configureCase()` template method hook and `agentWorker()` helper for building LLM-backed workers |
