...
class LifeCapabilitiesTest {

    @Test
    void allConstantsNonNullAndNonBlank() throws IllegalAccessException {
        for (Field field : LifeCapabilities.class.getDeclaredFields()) {
            if (field.getType() == String.class) {
                String value = (String) field.get(null);
                assertThat(value)
                        .as("LifeCapabilities.%s", field.getName())
                        .isNotNull()
                        .isNotBlank();
            }
        }
    ...
    @Test
    void allExpectedCapabilitiesPresent() throws IllegalAccessException {
        List<String> values = new ArrayList<>();
        for (Field field : LifeCapabilities.class.getDeclaredFields()) {
            if (field.getType() == String.class) {
                values.add((String) field.get(null));
